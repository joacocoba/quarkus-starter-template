...
public record CreateTransactionRequest(
    ...
    @NotBlank(message = "Currency is required") @Size(min = 3, max = 3, message = "Currency must be exactly 3 characters") String currency,
    @NotBlank(message = "Origin account number is required") @Size(
            min = 8,
            max = 20,
            message = "Origin account number must be between 8 and 20 characters")
        String originAccountNumber,
    @NotBlank(message = "Destination account number is required") @Size(
            min = 8,
            max = 20,
            message = "Destination account number must be between 8 and 20 characters")
